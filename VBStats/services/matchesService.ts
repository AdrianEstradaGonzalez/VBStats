 ...
 * Matches Service
 *
 * The server scopes every match to the session owner, so `user_id` filters are no
 * longer sent. The argument is kept so existing call sites keep compiling.
 */
...
export const matchesService = {
  ...
  getById: async (id: number): Promise<Match> => {
    const response = await apiFetch(`${API_ENDPOINTS.matches}/${id}`);
    if (!response.ok) throw new Error('Failed to fetch match');
    return response.json();
  },

  create: async (data: MatchCreate): Promise<Match> => {
    const response = await apiFetchJson(API_ENDPOINTS.matches, 'POST', data);
    if (!response.ok) throw new Error('Failed to create match');
    return response.json();
  },

  update: async (id: number, data: MatchUpdate): Promise<Match> => {
    const response = await apiFetchJson(`${API_ENDPOINTS.matches}/${id}`, 'PUT', data);
    if (!response.ok) throw new Error('Failed to update match');
    return response.json();
  },

  delete: async (id: number): Promise<void> => {
    const response = await apiFetch(`${API_ENDPOINTS.matches}/${id}`, { method: 'DELETE' });
    if (!response.ok) throw new Error('Failed to delete match');
  },

  // Get match statistics
  getStats: async (matchId: number): Promise<MatchStatsSummary> => {
    const response = await apiFetch(`${API_ENDPOINTS.matches}/${matchId}/stats`);
  ...
  saveMatchState: async (matchId: number, state: MatchState): Promise<void> => {
    const response = await apiFetchJson(`${API_ENDPOINTS.matches}/${matchId}/state`, 'PUT', state);
  ...
  getMatchState: async (matchId: number): Promise<MatchState | null> => {
    try {
      const response = await apiFetch(`${API_ENDPOINTS.matches}/${matchId}/state`);
      if (!response.ok) {
  ...
  deleteMatchState: async (matchId: number): Promise<void> => {
    const response = await apiFetch(`${API_ENDPOINTS.matches}/${matchId}/state`, { method: 'DELETE' });
  ...
  generateShareCode: async (matchId: number): Promise<{ share_code: string }> => {
    const response = await apiFetchJson(`${API_ENDPOINTS.matches}/${matchId}/generate-code`, 'POST');
    if (!response.ok) throw new Error('Failed to generate share code');
    return response.json();
  },

  // Get match by share code (public: this is the free-plan sharing feature)
  getByShareCode: async (code: string): Promise<Match> => {
    const response = await apiFetch(`${API_ENDPOINTS.matches}/by-code/${code}`);
