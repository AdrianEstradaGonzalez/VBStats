...
export default function TeamTrackingScreen({ 
  ...
  const renderBarChart = (data: { label: string; value: number; color: string }[], title: string) => {
    ...
    const barWidth = (CHART_WIDTH - 60) / data.length - gap;
      ...
      <View style={styles.chartContainer}>
        ...
        <Svg width={CHART_WIDTH} height={CHART_HEIGHT}>
          ...
          {data.map((d, i) => {
            ...
            const color = getMatchResultColor(d.result);
            return (
        ...
        <View style={styles.scatterLegend}>
          {(['win', 'loss', 'draw'] as MatchPerformance['result'][]).map(result => (
            <View key={result} style={styles.scatterLegendItem}>
              <View style={[styles.scatterDot, { backgroundColor: getMatchResultColor(result) }]} />
