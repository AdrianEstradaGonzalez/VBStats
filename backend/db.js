...
async function init() {
  ...
  try {
    ...
    try {
      await conn.query(`ALTER TABLE users ADD COLUMN last_login_at TIMESTAMP NULL AFTER session_token;`);
    } catch (err) {
      if (err.code !== 'ER_DUP_FIELDNAME') {
        console.error('Error adding last_login_at column to users:', err);
      }
    }
